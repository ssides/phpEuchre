<?php 
  ...
  if (!isset($_POST['r']) || !isAuthenticated($_POST['r']) || !isset($_POST['gameID']) || !isset($_POST['position']) || !isset($_POST['isFirst']) || strlen($_POST['position']) != 1 || strpos("OPLR", $_POST['position']) === false) {
  ...
  try {
    ...
    if ($isFirst) {
      ...
      mysqli_stmt_bind_param($stmt, 'ssss', $position, $position, $turn, $gameID);
    } else {
      $sql = "update `Game` set `Dealer`=?, `Turn`=?, `CardFaceUp`=null where `ID`=?";
      
      $stmt = mysqli_prepare($conn, $sql);
      if (!$stmt) { throw new Exception(mysqli_error($conn)); }
      mysqli_stmt_bind_param($stmt, 'sss', $position, $turn, $gameID);
    }
